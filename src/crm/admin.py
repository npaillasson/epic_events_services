...
class EventAdmin(admin.ModelAdmin):
    ...
    def render_change_form(
        ...
        if request.user.team == "1":
            context["adminform"].form.fields[
                "support_manager"
            ].queryset = User.objects.filter(team="3")
            context["adminform"].form.fields[
                "contract"
            ].queryset = Contract.objects.filter(is_signed=True)
    ...
    def client_manager(self, inst):
        return mark_safe(
            f"<a href={reverse('admin:accounts_user_change', args=[inst.contract.client.client_manager.id])}>{inst.contract.client.client_manager}</a>"
        )
    ...
    def client(self, inst):
        return mark_safe(
            f"<a href={reverse('admin:crm_client_change', args=[inst.contract.client.id])}>{inst.contract.client}</a>"
        )
...
class ClientAdmin(admin.ModelAdmin):
    ...
    def render_change_form(
        ...
        if request.user.team == "1":
            context["adminform"].form.fields[
                "client_manager"
            ].queryset = User.objects.filter(team="2")
